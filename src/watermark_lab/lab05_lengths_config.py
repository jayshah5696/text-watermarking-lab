"""Strict configuration for the Stage 5 natural-length evidence ladder."""
    ...
    def __post_init__(self) -> None:
        ...
        for name, expected in locked.items():
            if getattr(self, name) != expected:
                raise ValueError(f"Stage 5 length ladder locks {name} to {expected!r}")
        if not self.instruction_prefix.endswith("\n\n"):
            raise ValueError("instruction_prefix must end with one blank line")
        if len(self.prompts) != 12 or len({prompt.id for prompt in self.prompts}) != 12:
            raise ValueError("length ladder requires twelve unique prompts")
        if [prompt.max_new_tokens for prompt in self.prompts] != [200] * 4 + [400] * 4 + [800] * 4:
            raise ValueError("length ladder requires four prompts at each 200/400/800 cap")
        if any(
            not prompt.id or not prompt.id.isascii() or not prompt.text for prompt in self.prompts
        ):
            raise ValueError("prompt IDs and text must be non-empty")

    def prompt_seed(self, prompt_id: str) -> int:
        return derive_reference_seed(base_seed=self.base_seed, prompt_id=f"length-{prompt_id}")
...
def lengths_config_from_toml_bytes(payload: bytes) -> Lab05LengthsConfig:
    raw: Any = tomllib.loads(payload.decode())
    if not isinstance(raw, dict):
        raise TypeError("length config must be a TOML table")
    values = cast(Mapping[str, object], raw)
    expected = set(Lab05LengthsConfig.__dataclass_fields__)
    if set(values) != expected:
        raise ValueError("length config keys differ from the exact contract")
    raw_prompts = values["prompts"]
    if not isinstance(raw_prompts, list):
        raise TypeError("prompts must be an array of tables")
    prompts: list[LengthPrompt] = []
    for item in cast(list[object], raw_prompts):
        if not isinstance(item, dict):
            raise ValueError("each length prompt needs id, text, and max_new_tokens")
        prompt = cast(Mapping[str, object], item)
        if set(prompt) != {"id", "text", "max_new_tokens"}:
            raise ValueError("each length prompt needs id, text, and max_new_tokens")
        prompts.append(
            LengthPrompt(
                id=_text("prompt id", prompt["id"]),
                text=_text("prompt text", prompt["text"]),
                max_new_tokens=_integer("prompt max_new_tokens", prompt["max_new_tokens"]),
            )
        )
    kwargs: dict[str, object] = {}
    for name, field in Lab05LengthsConfig.__dataclass_fields__.items():
        if name == "prompts":
    ...
    return Lab05LengthsConfig(**cast(Any, kwargs))
