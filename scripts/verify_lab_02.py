#!/usr/bin/env python3
"""Recompute and validate the selected Stage 2 trace from its locked configuration."""
...
class VerificationError(RuntimeError):
    """A mismatch between Stage 2 configuration, provenance, and selected evidence."""
...
def main() -> int:
    ...
    try:
        ...
        if selected.config != config:
            raise VerificationError("selected configuration does not match configs/lab_02.toml")
        if selected.config_sha256 != config_sha256:
            raise VerificationError("selected config hash does not match configs/lab_02.toml")
        repository = Path(_git(Path.cwd(), "rev-parse", "--show-toplevel").strip())
        _git(repository, "cat-file", "-e", f"{selected.source_commit}^{{commit}}")
        source_config = _git(
            repository, "show", f"{selected.source_commit}:configs/lab_02.toml"
        ).encode()
        ...
        print(f"Stage 2 verification failed: {error}", file=sys.stderr)
        return 1
    print("Stage 2 trace verified against its config, source commit, and detector replay.")
    return 0
